...
static void constraint_destroy_notify(wl_listener* listener, void*)
{
    PointerConstraint& constraint
        = naoland_container_of(listener, constraint, destroy);

    auto& current_constraint = constraint.seat.current_constraint;
    if (current_constraint.has_value()
        && &current_constraint.value().get().wlr == &constraint.wlr) {
...
PointerConstraint::PointerConstraint(Seat& seat,
                                     wlr_pointer_constraint_v1& wlr) noexcept
    : listeners(*this)
    , seat(seat)
    , wlr(wlr)
{
