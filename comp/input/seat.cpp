...
{
    Seat& seat = naoland_container_of(listener, seat, new_input);
...
{
    Seat& seat = naoland_container_of(listener, seat, new_virtual_pointer);
    auto const* event
...
{
    Seat& seat = naoland_container_of(listener, seat, new_virtual_keyboard);
...
{
    Seat& seat = naoland_container_of(listener, seat, new_pointer_constraint);
    auto* wlr_constraint = static_cast<wlr_pointer_constraint_v1*>(data);

    auto const* focused_surface = seat.wlr->keyboard_state.focused_surface;
...
{
    Seat const& seat = naoland_container_of(listener, seat, request_cursor);
    auto const* event
        = static_cast<wlr_seat_pointer_request_set_cursor_event*>(data);

    wlr_seat_client const* focused_client
        = seat.wlr->pointer_state.focused_client;
    ...
    if (focused_client == event->seat_client) {
        ...
        wlr_cursor_set_surface(&seat.cursor.wlr, event->surface,
 ...
 * ignore such requests if they so choose, but in Naoland we always honor
 */
static void request_set_selection_notify(wl_listener* listener, void* data)
{
    Seat const& seat
        = naoland_container_of(listener, seat, request_set_selection);
    auto const* event
 ...
 */
Seat::Seat(Server& server) noexcept
    : listeners(*this)
    , server(server)
    , cursor(*this)
{
    ...
    wl_signal_add(&wlr->events.request_set_selection,
    ...
    wl_signal_add(&virtual_pointer_mgr->events.new_virtual_pointer,
                  &listeners.new_virtual_pointer);

    virtual_keyboard_mgr
    ...
    wl_signal_add(&virtual_keyboard_mgr->events.new_virtual_keyboard,
                  &listeners.new_virtual_keyboard);
    ...
    wl_signal_add(&pointer_constraints->events.new_constraint,
                  &listeners.new_pointer_constraint);
}
...
void Seat::apply_constraint(wlr_pointer const* pointer, double* dx,
                            double* dy) const
{
    if (!current_constraint.has_value()
    ...
    double confined_x = 0;
    double confined_y = 0;
    if (!wlr_region_confine(&current_constraint->get().wlr.region, x, y,
                            x + *dx, y + *dy, &confined_x, &confined_y)) {
...
bool Seat::is_pointer_locked(wlr_pointer const* pointer) const
{
    return current_constraint.has_value()
        && pointer->base.type == WLR_INPUT_DEVICE_POINTER
        && current_constraint->get().wlr.type
