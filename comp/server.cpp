...
{
    wlr_surface const* prev_surface = seat->wlr->keyboard_state.focused_surface;
    ...
    if (prev_surface) {
        ...
        if (auto const* xdg_previous
            = wlr_xdg_surface_try_from_wlr_surface(previous)) {
            assert(xdg_previous->role == WLR_XDG_SURFACE_ROLE_TOPLEVEL);
            wlr_xdg_toplevel_set_activated(xdg_previous->toplevel, false);
        } else if (auto* xwayland_previous
                   = wlr_xwayland_surface_try_from_wlr_surface(previous)) {
     ...
     */
    wlr_keyboard const* keyboard = wlr_seat_get_keyboard(seat->wlr);
    if (keyboard != nullptr) {
        wlr_seat_keyboard_notify_enter(
            seat->wlr, view->get_wlr_surface(), keyboard->keycodes,
            keyboard->num_keycodes, &keyboard->modifiers);
    }

    wlr_pointer_constraint_v1* constraint
        = wlr_pointer_constraints_v1_constraint_for_surface(
            seat->pointer_constraints, surface, seat->wlr);
    seat->set_constraint(constraint);
}

Surface* Server::surface_at(double const lx, double const ly, wlr_surface** wlr,
                            double* sx, double* sy) const
{
    /* This returns the topmost node in the scene at the given layout coords.
     * we only care about surface nodes as we are specifically looking for a
     * surface in the surface tree of a naoland_view. */
    wlr_scene_node* node = wlr_scene_node_at(&scene->tree.node, lx, ly, sx, sy);
    if (node == nullptr || node->type != WLR_SCENE_NODE_BUFFER) {
        return nullptr;
    }
    wlr_scene_buffer* scene_buffer = wlr_scene_buffer_from_node(node);
    wlr_scene_surface const* scene_surface
    ...
    *wlr = scene_surface->surface;
    /* Find the node corresponding to the naoland_view at the root of this
     * surface tree, it is the only one for which we set the data field. */
    wlr_scene_tree const* tree = node->parent;
...
{
    Server& server = naoland_container_of(listener, server, backend_new_output);
...
{
    auto const& event = *static_cast<wlr_output_power_v1_set_mode_event*>(data);
...
{
    Server& server
        = naoland_container_of(listener, server, xdg_shell_new_xdg_surface);
    auto const& xdg_surface = *static_cast<wlr_xdg_surface*>(data);

    if (xdg_surface.role == WLR_XDG_SURFACE_ROLE_TOPLEVEL) {
        new XdgView(server, *xdg_surface.toplevel);
    } else if (xdg_surface.role == WLR_XDG_SURFACE_ROLE_POPUP) {
        auto const* surface
            = static_cast<Surface*>(xdg_surface.popup->parent->data);
...
{
    Server& server
        = naoland_container_of(listener, server, activation_request_activation);
    auto const* event
        = static_cast<wlr_xdg_activation_v1_request_activate_event*>(data);

    auto const* xdg_surface
        = wlr_xdg_surface_try_from_wlr_surface(event->surface);
    if (xdg_surface == nullptr) {
        return;
    }

    auto* view = dynamic_cast<View*>(
        static_cast<Surface*>(xdg_surface->surface->data));
...
{
    Server& server = naoland_container_of(listener, server, drm_lease_request);
    auto* request = static_cast<wlr_drm_lease_request_v1*>(data);

    wlr_drm_lease_v1 const* lease = wlr_drm_lease_request_v1_grant(request);
    if (lease == nullptr) {
...
{
    Server& server
        = naoland_container_of(listener, server, output_layout_change);
    ...
    for (auto const* output : std::as_const(server.outputs)) {
...
{
    Server& server
        = naoland_container_of(listener, server, output_manager_apply);
    ...
    wl_list_for_each(head, &config.heads, link)
    {
        ...
        bool const enabled = head->state.enabled && !output.is_leased;
        bool const adding = enabled && !output.wlr.enabled;
        bool const removing = !enabled && output.wlr.enabled;
        ...
        if (enabled) {
            if (head->state.mode) {
                ...
                int32_t const width = head->state.custom_mode.width;
                int32_t const height = head->state.custom_mode.height;
                int32_t const refresh = head->state.custom_mode.refresh;
    ...
    : listeners(*this)
{
    ...
    wl_signal_add(&output_layout->events.change,
                  &listeners.output_layout_change);
    ...
    wl_signal_add(&output_manager->events.apply,
                  &listeners.output_manager_apply);

    output_power_manager = wlr_output_power_manager_v1_create(display);
    listeners.output_power_manager_set_mode.notify
        = output_power_manager_set_mode_notify;
    wl_signal_add(&output_power_manager->events.set_mode,
                  &listeners.output_power_manager_set_mode);
    ...
    wl_signal_add(&layer_shell->events.new_surface,
                  &listeners.layer_shell_new_layer_surface);
    ...
    wl_signal_add(&xdg_activation->events.request_activate,
                  &listeners.activation_request_activation);
    ...
    if (drm_manager != nullptr) {
        ...
        wl_signal_add(&drm_manager->events.request,
                      &listeners.drm_lease_request);
    }
