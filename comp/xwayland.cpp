...
static char const* atom_map[ATOM_LAST] = {
...
{
    XWayland& xwayland = naoland_container_of(listener, xwayland, ready);

    wlr_xwayland_set_seat(xwayland.wlr, xwayland.server.seat->wlr);

    xcb_connection_t* xcb_conn = xcb_connect(nullptr, nullptr);
    if (int32_t const err = xcb_connection_has_error(xcb_conn)) {
...
{
    XWayland& xwayland = naoland_container_of(listener, xwayland, new_surface);
    auto& xwayland_surface = *static_cast<wlr_xwayland_surface*>(data);

    new XWaylandView(xwayland.server, xwayland_surface);
}

XWayland::XWayland(Server& server) noexcept
    : listeners(*this)
    , server(server)
{
